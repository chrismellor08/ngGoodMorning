...
GoodMorning.controller('AlarmController', function ($window, $scope, $interval, $localStorage, $timeout, UserFactory, UserHubFactory, moment, geolocation, AlarmFactory) {
  ...
  $scope.newAlarmTime = {
    ...
    weatherType: null,
    weatherDelay: null,
    weatherMessage: "The weather looks great, and won't change your travel time!"
  };
  ...
  $scope.calculateTravelTime = function () {
    let handleResponse = function (data) {
      ...
      let newWeatherDelay = moment.duration($scope.newAlarmTime.weatherDelay);
      console.log("GHANN", newWeatherDelay);
      ...
      let weatheredDuration = newDuration.add(newWeatherDelay);
      let timeToWake = weatheredDuration.add(bufferTime);
      ...
      modal.setContent(`<h2>You're all set!</h2><h2>Your alarm has been set to ${$scope.personalInfo.finalAlarmTime}!</h2><br><h5>It will currently take you ${$scope.personalInfo.duration} to get to ${$scope.newAlarmTime.whereImGoing}.<br> It looks like you need need ${$scope.newAlarmTime.bufferTime} to get ready, and you need to be there by ${$scope.newAlarmTime.whenToBeThere}.<br>Also, ${$scope.newAlarmTime.weatherMessage}<br><br>We will keep checking to see if it will take longer to get there due to traffic or weather, and change your alarm time accordingly!<br><br>  Have a Good Morning!</h5>`);
  ...
  $scope.getWeather = () => {
    $.ajax({
      ...
      success: function (data) {
       ...
       console.log("HERRRE", $scope.newAlarmTime.weatherType);
        $scope.newAlarmTime.weatherType = data.weather[0].description;
        console.log("YEEHAW", $scope.newAlarmTime.weatherType);
      
        if($scope.newAlarmTime.weatherType.includes('clouds')){
          $scope.newAlarmTime.weatherDelay = "00:10";
          $scope.newAlarmTime.weatherMessage = "It looks like there's some rain in the forecast. Sometimes people have trouble driving in these conditions, so we've added 10 minutes to your drive time";

        } else if ($scope.newAlarmTime.weatherType.includes('snow')){
          $scope.newAlarmTime.weatherDelay = "00:20";
          $scope.newAlarmTime.weatherMessage = "It looks like there's some snow in the forecast. Sometimes people have trouble driving in these conditions, so we've added 20 minutes to your drive time";
        }
        else if ($scope.newAlarmTime.weatherType !== null){
          $scope.newAlarmTime.weatherDelay = "00:00";
          // $scope.newAlarmTime.weatherMessage = "The weather is great, and won't change your travel time!";
        }
        
        console.log("BAZANG!", $scope.newAlarmTime.weatherDelay);
      }
    });
  };
